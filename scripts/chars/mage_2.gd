...
@export var projectile_pool: ProjectilePool    # pool de proyectiles enemigos
...
func attack_and_cast() -> void:
	...
	# Configurar SFX de disparo secuenciado
	proj.sfx_index = _enemy_fire_sfx_idx
	_enemy_fire_sfx_idx = (_enemy_fire_sfx_idx + 1) % 3

	# Reactivar estados (añade al grupo, activa colisiones y reproduce SFX)
	if proj.has_method("reactivate"):
		proj.reactivate()

func _create_projectile() -> Projectile2:
	var p: Projectile2 = null
	if projectile_pool:
		p = projectile_pool.acquire() as Projectile2
	else:
		if projectile_scene == null:
			return null
		p = projectile_scene.instantiate() as Projectile2
		if p == null:
			return null
