...
@onready var player: Mage1    = get_node_or_null("%Mage_1") as Mage1
...
func _resolve_enemy_if_needed() -> void:
	...
	if n is EnemyBase: enemy = n
	if enemy != null: return
	var list := get_tree().get_nodes_in_group("enemy")
	for e in list:
		if e is EnemyBase: enemy = e
	if enemy != null: return
...
func _resolve_timer_and_panels() -> void:
	if timer_path != NodePath(""):
		...
		if t is TurnTimer: timer_ref = t
	if timer_ref == null:
		timer_ref = get_node_or_null("%TurnTimer") as TurnTimer
	if timer_ref == null:
	...
	if typing_panel_path != NodePath(""):
		...
		if tp is TypingPanel: typing = tp
	if typing == null:
	...
	if directions_panel_path != NodePath(""):
		...
		if dp is DirectionsPanel: defend = dp
	if defend == null:
