...
const STAGE1_PATHS := [
	...
	"res://scenes/stages/AdeptBattle.tscn",   # fallback si falta la primera
	"res://scenes/battle.tscn",               # último fallback
]

func _ready() -> void:
	# Conexiones de botones (a prueba de duplicados)
...
func _go_stage1() -> void:
	...
	if path == "":
		push_error("Menu: no encontré AdeptBattle. Revisa scenes/stages/AdeptBattle.tscn")
		return
	var tree: SceneTree = get_tree()
	if tree:
		tree.change_scene_to_file(path)

# Acepta Array sin tipar para no chocar con el literal de arriba
