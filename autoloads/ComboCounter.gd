...
func _scan_tree_for_sources() -> void:
	...
	if _typing == null:
		var by_class := get_tree().root.find_children("", "TypingPanel", true, false)
		if by_class.size() > 0 and by_class[0] is TypingPanel:
			_typing = by_class[0]
	if _typing == null:
	...
	if _direc == null:
		var by_class2 := get_tree().root.find_children("", "DirectionsPanel", true, false)
		if by_class2.size() > 0 and by_class2[0] is DirectionsPanel:
			_direc = by_class2[0]
	if _direc == null:
